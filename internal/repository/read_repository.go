...
// FindByDateRange retrieves tasks created within a date range
func (r *ReadRepository) FindByDateRange(ctx context.Context, startDate, endDate time.Time, status *domains.TaskStatus, page, pageSize int) ([]*domains.Task, int, error) {
	var args []interface{}
	argIndex := 1

	whereClause := fmt.Sprintf("WHERE created_at >= $%d AND created_at <= $%d", argIndex, argIndex+1)
	...
	// Count total records
	countQuery := fmt.Sprintf("SELECT COUNT(*) FROM tasks_read %s", whereClause)
	var total int
	err := r.db.QueryRowContext(ctx, countQuery, args...).Scan(&total)
	if err != nil {
		return nil, 0, fmt.Errorf("failed to count tasks: %w", err)
...
func (r *ReadRepository) CountByStatus(ctx context.Context, status domains.TaskStatus) (int, error) {
	query := `SELECT COUNT(*) FROM tasks_read WHERE status = $1`
...
// FindRecent retrieves the most recently created or updated tasks
func (r *ReadRepository) FindRecent(ctx context.Context, limit int) ([]*domains.Task, error) {
	query := `
		SELECT id, title, description, status, created_at, updated_at
		FROM tasks_read
...
func (r *ReadRepository) Search(ctx context.Context, searchTerm string, page, pageSize int) ([]*domains.Task, int, error) {
	...
	rows, err := r.db.QueryContext(ctx, query, searchPattern, pageSize, offset)
	if err != nil {
